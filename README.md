...
**模块化生图引擎 + 自管提示词库（gpt-image-2 驱动）。**
N 个 Markdown 模板 · 100 构图 · 383 画师 · 736 中国传统色 · 真人垫图，任意比例直出。
两个核心能力：**① 逆向反推别人的提示词** · **② 按 Markdown 规范把文档变成图**。

![templates](https://img.shields.io/badge/templates-N-success.svg)
![artists](https://img.shields.io/badge/artist%20styles-383-orange.svg)
![colors](https://img.shields.io/badge/traditional%20colors-736-c44.svg)
...
> **⚠️ 肖像权声明 · 人脸不可商用**
> 本 showcase 的人物 / 写真 / 角色类示例中出现的**两位真人的面部肖像，仅作技术演示，不可用于任何商业用途——即使在本 showcase 之内**。任何商业使用须另行取得本人肖像权授权。

---

## 看图说话

> 同一套引擎，一个模板一张图，主题围绕 **AI 企业应用、AI 落地与 AI 技术科普**。版面全部由模型自己排，人物用同一张脸演绎几十种风格，角色海报同一套版式演绎不同角色。
...
引擎 = 纯 CLI（Python 脚本）+ 一个 Markdown 提示词库。两个核心能力：

### ① 逆向工程：把别人的图反推成提示词（图 → prompt）

给一张你喜欢的图（别人的字效 / 海报 / 角色 / 设计），引擎按固定流程把它拆成一条**可复现、可迁移、能直接进库**的提示词。

**反推 → 验证 → 入库**

1. **反推** —— 八步拆解：文字内容 · 字体类型 · 字形结构 · 字效材质 · 颜色关系 · 光影空间 · 视觉气质 · 适合用途 → 产出【反推分析】+【可复制提示词】+【可替换模板（`{{变量}}`）】+【建议入库 front-matter】。
2. **验证** —— 用反推出的提示词调 gpt-image-2 出图，与原图比对（字形 / 字效 / 色 / 气质 / 版式），差距大改 1~2 轮重出。
3. **入库** —— 验证通过 → 存成一份 `prompt.md` 进库，从此可反复调用、换主题、换比例。

**版权红线**：游戏 / 品牌 / 他人 IP 角色 → 只反推「设计与版式系统」并迁移到**原创主体**（见画廊「角色海报 · 刊封编辑体」：从 LoL 角色海报反推版式，但出原创角色，不复刻原 IP）。

### ② Markdown 文档 → 生图规范（prompt.md 怎么写）

每个模板就是一份 `prompt.md`（Markdown + YAML front-matter）。引擎读它、填变量、调 gpt-image-2 出图——**任何人按这份规范写一份 `.md`，就能被引擎直接生成图**。

路径：`library/<类型>/<模板名>/prompt.md`
...
|---|---|
| `name` | 模板名 |
| `type` | `poster-viz` / `text-poster` / `portrait` / `text2img` / `infographic` / `style-transfer` |
| `variables` | 变量名列表，如 `[主题, 比例]` |
| `optional` | 可留空的变量（不填自动补空、由模型自判） |
| `ratio` | 默认比例，如 `9:16`（命令行 `--ratio` 可覆盖） |
| `tags` · `note` · `category_kind` | 标签 · 一句话定位 · 氛围类/字体类/信息类 |

**Body（正文 = 提示词）**

- 正文就是喂给 gpt-image-2 的完整提示词，用 `{{变量}}` 占位（如 `{{主题}}`、`{{比例}}`）。
- `{{比例}}` 由 `--ratio` 自动填；`optional` 变量不填时自动补空。
- 含文字的模板，结尾钉一条 **【文字硬规则】**：核心文字须笔画 / 拼写准确、不乱码，风格化只施加在正确字形之上。

**调用**

```bash
imagen.py --template <名> --var 主题=企业AI落地 --ratio 9:16            # dry-run 报成本，不出图
imagen.py --template <名> --var 主题=企业AI落地 --ratio 9:16 --confirm   # 确认后才出图
```

叠加层：`--person <真人>`（多图垫脸、可多人）· `--ref <图>`（垫图 / 风格迁移）· `--artist` · `--colors` · `--comp` · `--style`。

### 其他

- **六层自由组合**：模板 · 构图(100) · 画师(383) · 配色(736 中国传统色) · 风格片段 · 真人垫图，一行命令任意叠。
- **任意比例零裁切**：gpt-image-2 接受任意宽高（宽高都能 ÷16），按比例精确直出、不靠后期裁切。
- **中文准**：提示词点名汉字，海报中文干净可读、不乱码。
...
> 同一句提示词「远处是山，近处是深林和小溪」，383 位画家各画一张——`--artist 莫奈` 一换，风格全变。下面是其中 12 位。

<table>
<tr>
<td width="25%"><img src="gallery/painters/vincent-van-gogh.webp" alt="梵高"><br><sub><b>梵高</b></sub></td>
<td width="25%"><img src="gallery/painters/claude-monet.webp" alt="莫奈"><br><sub><b>莫奈</b></sub></td>
<td width="25%"><img src="gallery/painters/katsushika-hokusai.webp" alt="葛饰北斋"><br><sub><b>葛饰北斋</b></sub></td>
<td width="25%"><img src="gallery/painters/alphonse-mucha.webp" alt="穆夏"><br><sub><b>穆夏</b></sub></td>
</tr>
<tr>
<td width="25%"><img src="gallery/painters/rembrandt.webp" alt="伦勃朗"><br><sub><b>伦勃朗</b></sub></td>
<td width="25%"><img src="gallery/painters/pablo-picasso.webp" alt="毕加索"><br><sub><b>毕加索</b></sub></td>
<td width="25%"><img src="gallery/painters/hayao-miyazaki.webp" alt="宫崎骏"><br><sub><b>宫崎骏</b></sub></td>
<td width="25%"><img src="gallery/painters/gustav-klimt.webp" alt="克里姆特"><br><sub><b>克里姆特</b></sub></td>
</tr>
<tr>
<td width="25%"><img src="gallery/painters/mondrian.webp" alt="蒙德里安"><br><sub><b>蒙德里安</b></sub></td>
<td width="25%"><img src="gallery/painters/salvador-dali.webp" alt="达利"><br><sub><b>达利</b></sub></td>
<td width="25%"><img src="gallery/painters/andy-warhol.webp" alt="安迪·沃霍尔"><br><sub><b>安迪·沃霍尔</b></sub></td>
<td width="25%"><img src="gallery/painters/m-c-escher.webp" alt="埃舍尔"><br><sub><b>埃舍尔</b></sub></td>
</tr>
</table>
...
> 取自中国传统色卡，按色名+HEX 注入提示词上色——`--colors 黛蓝,朱砂红` 或 `--palette 青绿山水`。下面是其中 12 张色卡（每张含 HEX/RGB/CMYK + 配色推荐 + 色彩小识 + 应用示例）。

<table>
<tr>
<td width="25%"><img src="gallery/colors/胭脂红.jpg" alt="胭脂红"><br><sub><b>胭脂红</b> · #F03F24</sub></td>
<td width="25%"><img src="gallery/colors/藤黄.jpg" alt="藤黄"><br><sub><b>藤黄</b> · #FFD111</sub></td>
<td width="25%"><img src="gallery/colors/群青.jpg" alt="群青"><br><sub><b>群青</b> · #1772B4</sub></td>
<td width="25%"><img src="gallery/colors/黛蓝.jpg" alt="黛蓝"><br><sub><b>黛蓝</b> · #2A3C5C</sub></td>
</tr>
<tr>
<td width="25%"><img src="gallery/colors/月白.jpg" alt="月白"><br><sub><b>月白</b> · #EEF7F2</sub></td>
<td width="25%"><img src="gallery/colors/鸦青.jpg" alt="鸦青"><br><sub><b>鸦青</b> · #424B50</sub></td>
<td width="25%"><img src="gallery/colors/檀紫.jpg" alt="檀紫"><br><sub><b>檀紫</b> · #381924</sub></td>
<td width="25%"><img src="gallery/colors/秋海棠红.jpg" alt="秋海棠红"><br><sub><b>秋海棠红</b> · #EC2B24</sub></td>
</tr>
<tr>
<td width="25%"><img src="gallery/colors/月白天青.jpg" alt="月白天青"><br><sub><b>月白天青</b> · #C6D7DB</sub></td>
<td width="25%"><img src="gallery/colors/淡绛红.jpg" alt="淡绛红"><br><sub><b>淡绛红</b> · #EC7696</sub></td>
<td width="25%"><img src="gallery/colors/靛青.jpg" alt="靛青"><br><sub><b>靛青</b> · #1661AB</sub></td>
<td width="25%"><img src="gallery/colors/淡赭.jpg" alt="淡赭"><br><sub><b>淡赭</b> · #BE7E4A</sub></td>
</tr>
</table>
...
> 每一种都能 `--comp <名>` 注入，让模型按指定方式组织画面。

| 构图 | 用途 | 构图 | 用途 |
|---|---|---|---|
| `三分法构图` | 主体置于三分线或其交点 | `九宫格构图` | 井字分割，主体落在交点 |
| `黄金分割构图` | 主体落在黄金比例分割点 | `对称构图` | 左右或上下镜像对称，稳定庄重 |
| `非对称构图` | 不对称但视觉平衡，灵动 | `均衡式构图` | 不同元素重量相互制衡达到平衡 |
| `上下构图` | 画面分上下两部分 | `前后构图` | 前后景纵深分层 |
| `水平构图` | 强调横向水平线，平静开阔 | `垂直构图` | 强调竖向线条，挺拔高耸 |
| `三角形构图` | 元素构成三角形，稳定 | `倒三角构图` | 倒置三角，不稳定的张力 |
| `圆形构图` | 元素环绕成圆，团聚饱满 | `椭圆形构图` | 椭圆排布，柔和包容 |
| `方形构图` | 方正排布，端正秩序 | `矩形构图` | 矩形框定主体 |
| `菱形构图` | 菱形排布，动感中带稳定 | `十字构图` | 横竖交叉成十字，中心突出 |
| `T形构图` | 元素呈T字布局 | `L形构图` | 元素呈L字布局，留出大块空白 |
| `S形构图` | 主体或视线沿S曲线蜿蜒，优雅流动 | `C形构图` | 半环抱C形，引导聚焦 |
| `Z形构图` | 视线沿Z字折返扫读 | `V形构图` | 收束于底点的V形 |
| `X形构图` | 两条对角线交叉于中心 | `放射式构图` | 元素从中心向外放射 |
| `向心式构图` | 元素向中心汇聚 | `离心式构图` | 元素背离中心向外散开 |
| `环绕式构图` | 元素围绕中心主体 | `框架式构图` | 用前景元素（门窗枝叶）框住主体 |
| `开放式构图` | 主体延伸出画外，外向开放 | `封闭式构图` | 元素收于画内，完整自足 |
| `满版构图` | 主体铺满整个画面 | `留白式构图` | 大面积留白衬托主体 |
| `密集式构图` | 元素密集填充画面 | `疏朗式构图` | 元素稀疏，通透有空气 |
| `疏密对比构图` | 疏与密的区域形成对比 | `单点构图` | 单一视觉点 |
| `双点构图` | 两个视觉点相互呼应 | `多点构图` | 多个视觉点分布 |
| `单点聚焦构图` | 全部元素引向一个焦点 | `多点平衡构图` | 多个焦点相互平衡 |
| `群组式构图` | 元素成组排列 | `分组式构图` | 内容分成若干组 |
| `聚合式构图` | 元素聚拢成团块 | `分散式构图` | 元素均匀散布全画 |
| `集中式构图` | 视觉重量集中一处 | `扩散式构图` | 由内向外扩散 |
| `层叠式构图` | 元素层层叠压 | `阶梯式构图` | 元素呈阶梯递进 |
| `重复式构图` | 同类元素重复排列 | `渐变式构图` | 大小色彩或形状渐次变化 |
| `节奏式构图` | 元素有节拍地排布 | `韵律式构图` | 重复中带变化的韵律 |
| `对比式构图` | 强烈对比制造张力 | `呼应式构图` | 元素彼此呼应 |
| `镜像式构图` | 镜面对称或倒影 | `轴线式构图` | 沿一条轴线组织元素 |
| `中轴构图` | 主体落在中轴线上 | `偏轴构图` | 轴线偏离中心 |
| `网格式构图` | 规整网格排布 | `模块化构图` | 模块单元拼合 |
| `分栏式构图` | 竖向分栏排版 | `分割式构图` | 画面被块面分割 |
| `区块式构图` | 内容分区块组织 | `矩阵式构图` | 行列矩阵排布 |
| `棋盘式构图` | 棋盘格交错 | `卡片式构图` | 内容装入卡片单元 |
| `拼贴式构图` | 多素材拼贴 | `蒙太奇构图` | 多画面并置组接 |
| `引导线构图` | 用线条引导视线 | `透视构图` | 透视线汇聚增强空间 |
| `纵深构图` | 强调前中后纵深 | `近大远小构图` | 透视近大远小 |
| `前景构图` | 突出前景元素 | `中景构图` | 以中景为主体 |
| `背景构图` | 强调背景氛围 | `层次构图` | 分明的前中后层次 |
| `空间构图` | 强调空间关系 | `深度构图` | 营造画面深度 |
| `斜线构图` | 斜线制造动感 | `曲线构图` | 曲线柔美流动 |
| `折线构图` | 折线转折有力 | `波浪线构图` | 波浪起伏的律动 |
| `交叉线构图` | 线条交叉聚焦 | `平行线构图` | 平行线秩序延展 |
| `重心构图` | 视觉重心稳定居中 | `偏重心构图` | 重心偏置制造张力 |
| `视觉中心构图` | 强主体占据视觉中心 | `视觉动线构图` | 安排清晰的阅读动线 |
| `动势构图` | 强烈的运动趋势 | `静态构图` | 平稳静止 |
| `动静对比构图` | 动与静并置对比 | `开合构图` | 开与合的呼吸关系 |
| `虚实构图` | 虚实结合（焦内清焦外糊） | `明暗构图` | 明暗对比塑造主体 |
| `色块构图` | 大色块结构画面 | `色彩对比构图` | 强色彩对比 |
| `大小对比构图` | 大小悬殊的对比 | `主次构图` | 主次分明的层级 |
...
这套引擎是 **Astralune** 的内部能力，引擎与提示词不外发。想用它给你的品牌 / 项目出图 → **[astralune.ai](https://astralune.ai)**

---

## 致谢

画家风格示例图来自 [Qiaomu Artist Style](https://github.com/joeseesun/qiaomu-artist-style)；中国传统色卡来自 [中国传统色](https://github.com/nevertoday/zhongguo-traditional-colors)。仅作能力展示。

© 2026 Astralune · 保留所有权利。商业使用请联系。

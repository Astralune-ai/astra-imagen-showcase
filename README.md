...
**一个会自我生长的生图引擎。**
你给意图，它照你的要求现写提示词，自由调度 81 模板 · 100 构图 · 383 画师 · 736 中国传统色 · 真人垫图，任意比例直出专业级画面；
每一条好提示词都沉淀回库，让它越用越懂你、越用越强。排版交给模型，因为它的审美足够强。

![templates](https://img.shields.io/badge/templates-81-success.svg)
...
astra-imagen 的本事，是把一张图拆成**可自由组合的几层**，每一层都能单独换、叠加用——再交给 gpt-image-2 一次成图。

### 1. 六层模块，自由组合

| 层 | 是什么 | 怎么用 |
|---|---|---|
| **固有提示词** | 81 个手调的完整构图模板 | `--template 诗性编辑视觉` |
| **不同构图** | 100 种构图法 | `--comp S形构图` |
| **不同画师风格** | 383 位画家的视觉语言 | `--artist 莫奈,梵高` |
| **配色** | 736 个中国传统色 + 策展方案 | `--colors 黛蓝,朱砂红` |
| **风格片段** | 轻量可叠加的质感 | `--style 胶片暖调` |
| **人物垫图** | 真人多图综合、保脸放开 | `--person <名>` |
...
这是和别人最不一样的地方。astra-imagen 不只是套死模板——它内置了**母版（提示词写作配方）**：氛围类一套、重型编辑类一套、反推类一套。你给一个用途和一句话，它能照你的 spec **现写一条全新的、具体的提示词**，出图后还能把这条好提示词**存回库里**变成新模板。

**反过来也行——给它一张你喜欢的图，它能逆向「反推」出一条可复现、可迁移的提示词**（八步拆解 → 出图验证 → 存回库），把别处的好设计变成你自己能反复调用的模板。遇到游戏/品牌 IP，只反推它的设计与版式、迁移到原创主体（见「角色海报 · 刊封编辑体」）。

### 3. 真任意比例，直接出图，零裁切

gpt-image-2 接受任意宽高（只要都能被 16 整除）。引擎按你要的比例算出精确尺寸**直接生成**——9:16、16:9、小红书、手机壁纸、X 封面，模型自己按整张画布排版，**不靠后期裁切**，边上的字和主体一个不丢。

### 4. 中文写得准

提示词点名要写的准确汉字，海报上的中文干净可读、不乱码。

### 5. 越用越厚的「复利库」

每条你创造的好提示词都能沉淀回库，模板、配色、构图、画师会随使用不断累积。**这是我们做这个技能的初衷——一个能自我沉淀、不断长本事的生图引擎；用它，去反哺它。**
...
这套生图引擎是 **Astralune** 的内部能力——81 个手调模板、383 位画师风格、736 中国传统色、任意比例直出，会按你的需求现写提示词。引擎与提示词不外发。

想用它给你的品牌 / 项目 / 内容出图，→ **[astralune.ai](https://astralune.ai)**
